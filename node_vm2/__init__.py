...
class NodeVMModule:
    ...
    def call_js_func(self, ids, *args):
        """Call a function member.
    ...
    def set_member(self, member, arg):
        return self.communicate({
            "action": "setMember",
            "member": member,
            "arg": arg
...
class VMServer:
        ...
        def reader():
            for data in self.process.stdout:
                ...
                try:
                    ...
                    self.close()
                    return
                if ('outputEncode' in data):
                    if 'vmId' in data:
                        data = self.str_to_obj(data, data['vmId'])
                    else:
                        data = self.str_to_obj(data, None)
    ...
    def communicate(self, data):
        ...
        """
        id = self.generate_id()
        data = self.obj_to_str(data)
        data['inputEncode'] = True
        data["id"] = id
    ...
    def obj_to_str(self, obj):
        if type(obj) == dict:
            ...
            try:
                ...
                pass
            if fun_list not in fun_list:
                fun_list.append(obj)
            return 'pyfun' + str(fun_list.index(obj))
        elif type(obj) == str:
            return 'str' + obj
        else:
            return obj

    def str_to_obj(self, obj, vm_id):
        if type(obj) == dict:
            for i in obj:
                obj[i] = self.str_to_obj(obj[i], vm_id)
            return obj
        elif type(obj) == list:
            for i in range(len(obj)):
                obj[i] = self.str_to_obj(obj[i], vm_id)
            ...
            if (str(obj).find('str') == 0):
                return obj[3:]
            if (str(obj).find('pyfun') == 0):
                return fun_list[int(obj[5:])]
            if (str(obj).find('jsfun') == 0):
                def func(*args):
                    tmp = self.communicate({
                        "action": "callJsFunc",
                        'ids': int(obj[5:]),
                        "args": args,
                        'vmId': vm_id
